...
def refresh_codice(short_id: str) -> None:
    ...
    Idempotente: il loader fa upsert. Si può schedulare settimanalmente.
    """
    logger.info("refresh_codice.start", short_id=short_id)
    import asyncio

    from avvocato_ingestion.cli import _run_normattiva

    asyncio.run(_run_normattiva(short_id))

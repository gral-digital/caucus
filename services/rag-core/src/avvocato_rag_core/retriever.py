...
class HybridRetriever:
    ...
    async def retrieve(self, query: RetrievalQuery) -> RetrievalResult:
        ...
        # Se più collection: normalizza per RRF inter-collection (score sono già RRF intra).
    ...
    def _build_filters(self, query: RetrievalQuery) -> Iterable[qm.FieldCondition]:
        ...
        if query.effective_at:
            ref_dt = datetime.combine(query.effective_at, time.min, tzinfo=timezone.utc)
            # effective_from <= ref_dt AND (effective_to IS NULL OR effective_to > ref_dt)
            yield qm.FieldCondition(
                key="effective_from", range=qm.DatetimeRange(lte=ref_dt)
            )
            ...
            yield qm.FieldCondition(
                key="effective_to", range=qm.DatetimeRange(gt=ref_dt)
            )
    ...
    def _to_hit(point: qm.ScoredPoint) -> RetrievalHit:
        ...
        partition_id = UUID(str(payload["partition_id"])) if "partition_id" in payload else UUID(int=0)
        ...
        return RetrievalHit(
            ...
            comma_id=comma_id,
            citation=citation,
            text=payload.get("text", ""),  # opzionale: alcuni indici non salvano il testo nel payload

...
19. **SSE fragile**: `done` emesso solo su `finish_reason`: stream chiuso senza → UI bloccata su "streaming"; `str(exc)` rimandato al client (leak dettagli provider); retrieval fuori dal try; probabile **leak di connessioni DB** (dependency `yield` chiusa prima dello streaming, lavoro DB dentro il generatore; vedi `deps.py:36-38`).
...
Inoltre: tutte le 23 fonti in un'unica collection `codici`, mentre `CorpusFilter.LEGGI` e `CASSAZIONE` sono enum vuoti.
